...
import { sendBroadcastNotification, sendQueueItem, getNotificationTargets, getNotificationEstimate, NotificationQueueItem, getAppConfig, updateAppConfig, AppConfig } from "@/lib/api-client"
...
export function AdminNotifications() {
  ...
  const handleToggle = async (key: 'auto_queue_offers' | 'auto_queue_partners', checked: boolean) => {
    if (!config) return
    ...
    try {
      ...
      toast({
        ...
        description: "Automatic queue settings updated successfully.",
      })
    } catch (error) {
        ...
        <Card className="p-4 bg-muted/30">
          <div className="flex flex-col md:flex-row gap-4 md:items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-medium">Automatic Queue Toggles</h3>
                <p className="text-xs text-muted-foreground">Toggle automatic notifications generated by system events</p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-6">
              <div className="flex items-center gap-2">
                <Switch
                  id="auto-partners"
                  checked={config.auto_queue_partners}
                  onCheckedChange={(checked) => handleToggle('auto_queue_partners', checked)}
                />
                <Label htmlFor="auto-partners" className="text-xs font-normal cursor-pointer">
                  Auto-queue Partner Drops
                </Label>
              </div>
              <div className="flex items-center gap-2">
                <Switch
                  id="auto-offers"
                  checked={config.auto_queue_offers}
                  onCheckedChange={(checked) => handleToggle('auto_queue_offers', checked)}
                />
                <Label htmlFor="auto-offers" className="text-xs font-normal cursor-pointer">
                  Auto-queue Offer Alerts
                </Label>
              </div>
            </div>
          </div>
